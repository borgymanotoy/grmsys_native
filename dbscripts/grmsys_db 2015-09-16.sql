...
-- Script date 9/15/2015 11:35:05 PM
...
CREATE TABLE item_sales_details (
  ...
  UNIQUE INDEX UK_item_sales_details_item_sal (item_sales_id),
  UNIQUE INDEX unique_item_sales_details (item_sales_id, item_id)
)
ENGINE = INNODB
CHARACTER SET utf8
...
VIEW vw_item_sales
AS
	select `isl`.`id` AS `id`,`isd`.`item_id` AS `item_id`,`isd`.`qty` AS `item_quantity`,`isd`.`unit_total` AS `unit_total`,`isd`.`remarks` AS `remarks`,`isl`.`sales_date` AS `sales_date`,`isl`.`grand_total_amount` AS `grand_total_amount`,`isl`.`member_id` AS `member_id`,if(isnull(`isl`.`member_id`),'Anonymous',trim(concat(ifnull(`m`.`firstname`,''),' ',if((`m`.`middlename` is not null),concat(substr(`m`.`middlename`,1,1),'. '),''),ifnull(`m`.`lastname`,'')))) AS `customer_name`,`isl`.`user_id` AS `user_id` from ((`item_sales` `isl` left join `item_sales_details` `isd` on((`isd`.`item_sales_id` = `isl`.`id`))) left join `member` `m` on((`m`.`id` = `isl`.`member_id`)));
...
-- Table grmsys_db.item_sales_details does not contain any data (it is empty)
