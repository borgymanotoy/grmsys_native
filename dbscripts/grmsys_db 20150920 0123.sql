...
-- Script date 9/20/2015 1:23:44 AM
...
AS
	select `vw_sold_items`.`sales_group` AS `sales_group`,sum(`vw_sold_items`.`unit_total`) AS `total` from `vw_sold_items` group by `vw_sold_items`.`sales_group`;
